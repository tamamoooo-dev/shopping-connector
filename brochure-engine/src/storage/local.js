...
export function createMemoryMetadataStore() {
  ...
  return {
    ...
    async getById(id) {
      // Mirrors the D1 metadataStore: SELECT * FROM brochures WHERE id = ?.
      if (!id) return null;
...
export function createMemoryOfferStore() {
  ...
  return {
    ...
    async byFlyer(store, region, flyerRef) {
      // Mirrors the D1 offerStore: SELECT * FROM offers WHERE store=? AND
      // region=? AND flyer_ref=? LIMIT 2000 (the hotspots read-path join).
      return [...rows.values()]
