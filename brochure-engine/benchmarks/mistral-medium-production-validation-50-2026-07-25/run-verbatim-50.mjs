// Full 50-crop re-run under the Verbatim prompt.
//
// The prompt is read verbatim out of run-prompt-b.mjs — not retyped, not edited,
// not merged with anything — so it is provably the same string that produced the
// 6/10 and 9/10 trial results. Everything else (frozen bytes, model, temperature,
// top_p, reasoning, JSON schema, one request per crop) is identical to the
// baseline run in model-results.json.
...
const RESULTS_PATH = join(HERE, 'verbatim-results.json');
...
const PROMPT = (await readFile(join(HERE, 'run-prompt-b.mjs'), 'utf8'))
...
// Identical canonicalisation to run-benchmark.mjs.
...
const frozen = await loadJson(join(HERE, 'frozen-sample.json'));
...
for (const sample of frozen.samples) {
  ...
  process.stdout.write(`${sample.index}/50 ${sample.store}: ${row.ok ? 'ok' : row.error}\n`);
  await sleep(400);
}
